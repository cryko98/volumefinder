...
export const getTokenHolders = async (
  ...
  return retryRequest(async () => {
    ...
    const top10Total = sorted.slice(0, 10).reduce(
      (sum: number, acc: any) => sum + (acc.amount || 0),
      0
    );
...
export const checkMintAuthority = async (
  mintAddress: string
): Promise<{ isFrozen: boolean; authorityExists: boolean }> => {
  if (!API_BASE) return { isFrozen: false, authorityExists: true };
  ...
  return retryRequest(async () => {
    ...
    if (response.data?.error) return { isFrozen: false, authorityExists: true };
