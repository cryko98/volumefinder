...
export const getTrendingTokens = async (): Promise<TokenPair[]> => {
  return retryRequest(async () => {
    ...
    return response.data.pairs
      .filter((pair: any) =>
        ['raydium', 'pumpswap', 'meteora'].includes(pair.dexId?.toLowerCase()) &&
        (pair.marketCap?.usd || 0) >= 40000
      )
      .map((pair: any) => parsePair(pair));
  });
};

export const getTokenByAddress = async (address: string): Promise<TokenPair | null> => {
  return retryRequest(async () => {
...
export const getTokensByAddresses = async (addresses: string[]): Promise<TokenPair[]> => {
...
const parsePair = (pair: any): TokenPair => ({
